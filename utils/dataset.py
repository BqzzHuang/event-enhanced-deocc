...
class dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        str_e = '/home/event/projects_dir/dataset/Occlusion-400/occlusion/board/024/event_1.15_003576892.npy'
        gt_img = torch.from_numpy(np.array(Image.open(str1 + str2[1:])).transpose([2,0,1])) / 255
        #gt_img = torch.from_numpy(np.array(Image.open('/home/event/projects_dir/dataset/Occlusion-400/occlusion/board/024/image.jpg')).transpose([2, 0, 1])) / 255
        ...
        #img = torch.from_numpy(np.load('/home/event/projects_dir/dataset/Occlusion-400/occlusion/board/024/img.npy')) / 255
        mask = torch.from_numpy(np.load(os.path.join(folder, 'mask.npy')))
        #mask = torch.from_numpy(np.load('/home/event/projects_dir/dataset/Occlusion-400/occlusion/board/024/mask.npy'))
        # mask = self.event2vox(np.load(str_e))
        #print(2)
        #print(mask.shape)
        #print(event_vox.shape)
