...
class dataset_nomask(data.Dataset):
    ...
    def __getitem__(self, idx):
        '''
        str1 = '/home/event/projects_dir/dataset/Occlusion-400'
        str2 = self.gt_file[idx]
        gt_img = torch.from_numpy(np.array(Image.open(str1 + str2[1:])).transpose([2,0,1])) / 255
        folder = self.img_folder[idx]
        folder = str1 + folder[1:]
        event_vox = torch.from_numpy(np.load(os.path.join(folder, 'e.npy')))
        img = torch.from_numpy(np.load(os.path.join(folder, 'img.npy'))) / 255
        '''

        str_e = '/home/event/projects_dir/dataset/data/0510_9_output/event.npy'
        gt_img = torch.from_numpy(np.array(Image.open('/home/event/projects_dir/dataset/output_folder/gt.png'))) / 255
        gt_img = torch.unsqueeze(gt_img, dim=0)
        event_vox = self.event_grid(np.load(str_e))
        img = torch.from_numpy(np.load('/home/event/projects_dir/dataset/data/0510_9_output/frame.npy')) / 255

        img, gt_img, event_vox = self.data_augmentation(img, gt_img, event_vox)

        return event_vox, img, gt_img
    ...
    def event2vox(self, event):
        ...
        t, p, x, y = event.t() #juzhenzhuanzhi
        t = t.long()
        time_max = t.max()
        time_min = t.min()
    ...
    def data_augmentation(self, input_image, gt_image, event_grid, crop_size=(256, 256)):
        ...
        #event_grid[0,...] = torch.zeros(size=event_grid[1,...].shape)
        img_channel = input_image.shape[0]
        gt_channel = gt_image.shape[0]
        tb = event_grid.shape[-1]
        pos = event_grid[0,...].permute([2,0,1]) #HWC->CHW
        neg = event_grid[1,...].permute([2,0,1])
        x = torch.cat([input_image, gt_image, pos, neg], dim=0)
        ...
        return input_image1, gt_image1, event_grid1
